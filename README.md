...
이 프로젝트는 자바(Java)로 개발된 2D 게임으로, 플레이어는 몬스터(enemy)가 마을로 들어오는 것을 방어해야 합니다. 문(gate)을 파괴하여 몬스터가 생성되지 않도록 막고, 플레이어의 목숨을 유지하며 제한된 시간 동안 마을을 지켜내는 게임입니다.
...
- **동적 문 시스템**: 문은 랜덤 위치에서 생성되며 체력(HP)을 가지고 있습니다. 플레이어는 문을 공격하여 파괴해야 합니다.
...
1. 이 저장소를 클론합니다:
   ```bash
   git clone https://github.com/your-username/monster-gate-defense.git
   cd monster-gate-defense
